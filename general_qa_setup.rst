...
2. Run the test suite using the Makefile. 
   ...
   An optional parameter doc_dir can be set when running the makefile. This parameter indicates the location of a directory where Sphinx documents will be created. If doc_dir is not set the qa-toolbox will automatically create a doc folder within the directory.
...
4. To look at the documentation generated by the test suite, cd into the document directory. For example, if the default doc_dir was chosen:

   .. code-block:: bash

     $ cd docs

   Setup sphinx in documentation directory and follow setup instructions.

   .. code-block:: bash

     $ sphinx-quickstart

   Note: This can be done before the test suite is run in order to avoid sphinx creating another master file index.rst. The existing index.rst will not be overwritten but Sphinx will create a new file name that can then be deleted.
